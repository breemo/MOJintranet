...
{
    public class StatusAskAnExpertDataManager
    {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                using (SPSite oSite = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb oWeb = oSite.RootWeb)
                    {
                        if (oWeb != null)
                        {
                            SPList lst = oWeb.GetListFromUrl(oSite.Url + SharedConstants.StatusAskAnExpertUrl);
                            if (lst != null)
                            {
        ...
        public string GetByID(int id, string languageCode)
        {
            string resalt="";
            SPSecurity.RunWithElevatedPrivileges(delegate ()
        {
                ...
                using (SPWeb oWeb = oSite.RootWeb)
                {
                    if (oWeb != null)
                    {
                        ...
                        if (lst != null)
                        {
                            ...
                            if (languageCode == "ar")
                            {
                                resalt = Convert.ToString(Item["Title"]);
                            }
                            else
                            {
                                resalt = Convert.ToString(Item["TitleEN"]);
                            }
                        }
                    }
                }
