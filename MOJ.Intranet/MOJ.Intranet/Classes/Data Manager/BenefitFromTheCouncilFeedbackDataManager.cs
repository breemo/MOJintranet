...
{
    public class BenefitFromTheCouncilFeedbackDataManager
    {
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite oSite = new SPSite(SPContext.Current.Site.Url))
                    {
                        using (SPWeb oWeb = oSite.RootWeb)
                        {
                            if (oWeb != null)
                            {
                                SPList lst = oWeb.GetListFromUrl(oSite.Url + SharedConstants.BenefitFromTheCouncilFeedbackUrl);
                                if (lst != null)
                                {
                                ...
                                {
                                    title = Convert.ToString(Item["TitleEN"]);
